function Get-MerakiOrganizationWirelessControllerDevicesInterfacesL2UsageHistoryByInterval {
    <#
    .SYNOPSIS
    Retrieves L2 interface usage history grouped by interval for wireless controller devices in a Meraki organization.

    .DESCRIPTION
    Get-MerakiOrganizationWirelessControllerDevicesInterfacesL2UsageHistoryByInterval calls the Meraki Dashboard API to return Layer 2 interface usage history (by interval) for wireless controller devices in the specified organization. If OrganizationID is not provided, it attempts to resolve a single organization using Get-OrgID -AuthToken <AuthToken>. The function builds query parameters for time range, pagination, and device serial filtering, issues an authenticated GET request, and returns the parsed JSON response.
    ...
    Get-MerakiOrganizationWirelessControllerDevicesInterfacesL2UsageHistoryByInterval -AuthToken $MERAKIKEY -OrganizationID "123456" -t0 "2024-01-01T00:00:00Z" -t1 "2024-01-02T00:00:00Z"
    #>
    [CmdletBinding()]
    param (
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try { 
            ...
            $URL = "https://api.meraki.com/api/v1/organizations/$OrganizationID/wirelessController/devices/interfaces/l2/usage/history/byInterval?$queryString"
