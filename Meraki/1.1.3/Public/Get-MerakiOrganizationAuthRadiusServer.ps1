function Get-MerakiOrganizationAuthRadiusServer {
    <#
    .SYNOPSIS
    Retrieves the RADIUS authentication servers configured for a Meraki organization.

    .DESCRIPTION
    Calls the Cisco Meraki API to return the authentication RADIUS servers for the specified organization.
    ...
    .PARAMETER ServerID
    The unique identifier of the RADIUS server to retrieve. This parameter is mandatory.
    
    .EXAMPLE
    # Using an explicit organization ID
    Get-MerakiOrganizationAuthRadiusServers -AuthToken 'ABCDEF123456' -OrganizationID '123456' -ServerID '1291234781'

    .EXAMPLE
    # Let the helper determine the organization ID (only works when a single org is found)
    Get-MerakiOrganizationAuthRadiusServers -AuthToken 'ABCDEF123456' -ServerID '1291234781'
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/organizations/$OrganizationID/auth/radius/servers/$ServerID" -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat"
            return $response
        }
        catch {
        Write-Debug $_
