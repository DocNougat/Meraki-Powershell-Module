function Set-MerakiNetworkWirelessSSIDBonjourForwarding {
    ...
    $BonjourConfig = $BonjourConfig | ConvertTo-Json -Compress -Depth 4
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Put -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $BonjourConfig
        return $response
    }
    catch {
        Write-Debug $_
