function New-MerakiOrganizationWirelessDevicesRadsecCertificatesAuthority {
    <#
    .SYNOPSIS
    Creates the RadSec certificate authorities configuration for wireless devices in a Meraki organization.

    .DESCRIPTION
    Sends a PUT request to the Meraki Dashboard API to create the RadSec certificate authorities configuration for all wireless devices in the specified organization. The function accepts an API key, an organization identifier (optional — will be resolved when not supplied), and a JSON payload representing the certificate authorities configuration as expected by the Meraki API.
    ...
    New-MerakiOrganizationWirelessDevicesRadsecCertificatesAuthorities -AuthToken 'AKIA...' -OrganizationID '123456' -CertificateAuthoritiesConfig $CertificateAuthoritiesConfig
    ...
    - This function creates the RadSec certificate authorities configuration for the organization's wireless devices.
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Post -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $CertificateAuthoritiesConfig
            return $response
        }
        catch {
            Write-Debug $_
