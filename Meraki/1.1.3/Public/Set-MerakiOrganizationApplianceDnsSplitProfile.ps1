function Set-MerakiOrganizationApplianceDnsSplitProfile {
<#
.SYNOPSIS
Updates a Meraki Appliance DNS split profile for an organization.

.DESCRIPTION
Calls the Meraki Dashboard API to update a DNS split profile associated with the specified organization.
...
.PARAMETER profileId
String. Mandatory. The profile ID(s) to restrict results to. Multiple IDs can be passed as a comma-separated string.
...
Set-MerakiOrganizationApplianceDnsSplitProfile -AuthToken 'xxxxxxxxxxxxxxxxxxxx' -profileId '9182734651' -profileConfig $config
.LINK
...
#>
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Put -Uri $URI -headers $header -body $body -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat"
            return $response
        } catch {
            Write-Debug $_
            Throw $_
