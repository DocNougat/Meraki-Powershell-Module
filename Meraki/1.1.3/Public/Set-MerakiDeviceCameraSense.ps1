function Set-MerakiDeviceCameraSense {
    ...
    $CameraSenseConfig = $CameraSenseConfig | ConvertTo-Json -Compress -Depth 4
    ...
    #>
        ...
        try {
            ...
            $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/camera/sense"
    
            $response = Invoke-RestMethod -Method Put -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $CameraSenseConfig
            return $response
        }
        catch {
        Write-Debug $_
