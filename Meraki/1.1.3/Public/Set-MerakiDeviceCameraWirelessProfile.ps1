function Set-MerakiDeviceCameraWirelessProfile {
    ...
    $WirelessProfileConfig = $WirelessProfileConfig | ConvertTo-Json -Compress -Depth 4
    ...
    #>
        ...
        try {
            ...
            $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/camera/wirelessProfiles"
    
            $response = Invoke-RestMethod -Method Put -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $WirelessProfileConfig
            return $response
        }
        catch {
        Write-Debug $_
