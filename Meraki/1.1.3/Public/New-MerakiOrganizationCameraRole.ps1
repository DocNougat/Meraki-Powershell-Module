function New-MerakiOrganizationCameraRole {
    ...
    $CameraRoleConfig = $CameraRoleConfig | ConvertTo-Json -Compress -Depth 4
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Post -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $CameraRoleConfig
            return $response
        }
        catch {
        Write-Debug $_
