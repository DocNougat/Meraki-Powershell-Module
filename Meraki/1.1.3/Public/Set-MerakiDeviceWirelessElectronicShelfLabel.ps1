function Set-MerakiDeviceWirelessElectronicShelfLabel {
    ...
    try {
        ...
        $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/wireless/electronicShelfLabel"

        $response = Invoke-RestMethod -Method Put -Uri $url -Headers $header -Body $body -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat"
        return $response
    }
    catch {
        Write-Debug $_
