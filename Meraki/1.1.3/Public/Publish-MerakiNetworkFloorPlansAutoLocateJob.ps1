function Publish-MerakiNetworkFloorPlansAutoLocateJob {
<#
.SYNOPSIS
Publishes an existing Meraki floor plan auto-locate job for a specified network.

.DESCRIPTION
Sends a POST request to the Meraki Dashboard API to publish an auto-locate job associated with a network floor plan. The function supplies the provided API key in the X-Cisco-Meraki-API-Key header and returns the response from Invoke-RestMethod. Errors are written to the Debug stream and rethrown to the caller.
...
#>
    ...
    param (
        ...
        try {
            ...
            $url = "https://api.meraki.com/api/v1/networks/$NetworkID/floorPlans/autoLocate/jobs/$JobID/publish"
