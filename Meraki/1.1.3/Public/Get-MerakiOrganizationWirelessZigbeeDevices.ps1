function Get-MerakiOrganizationWirelessZigbeeDevices {
    ...
    (Optional) Boolean to filter results by enrollment status. Passed to the API as the isEnrolled query parameter.

    .PARAMETER search
    (Optional) String to search for specific Zigbee devices by name or serial number.
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try { 
            ...
            If ($isEnrolled) {
                $queryParams["isEnrolled"] = $isEnrolled
            }
            If ($search) {
            ...
            $URL = "https://api.meraki.com/api/v1/organizations/$OrganizationID/wireless/zigbee/devices?$queryString"
