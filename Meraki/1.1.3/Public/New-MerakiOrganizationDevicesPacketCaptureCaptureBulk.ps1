function New-MerakiOrganizationDevicesPacketCaptureCaptureBulk {
    <#
    .SYNOPSIS
    Creates bulk packet capture sessions on devices in a Meraki organization by calling the Meraki Dashboard API.

    .DESCRIPTION
    New-MerakiOrganizationDevicesPacketCaptureCaptureBulk posts a packet capture request to the Meraki Dashboard for the specified organization. If OrganizationID is not provided, the function attempts to resolve it via Get-OrgID using the provided API token. If multiple organizations are found, the function returns an informative message and does not call the API. The function constructs the necessary request headers and body, calls Invoke-RestMethod to create the capture, and returns the parsed API response. On failure the underlying exception is written to debug stream and re-thrown.
    ...
    New-MerakiOrganizationDevicesPacketCaptureCapture -AuthToken $apiKey -OrganizationID "123456" -PacketCaptureConfig $packetCaptureConfig
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $url = "https://api.meraki.com/api/v1/organizations/$OrganizationId/devices/packetCapture/captures/bulkCreate"
