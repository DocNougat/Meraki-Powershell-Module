function Update-MerakiOrganizationDevicesPacketCaptureSchedule {
    <#
    .SYNOPSIS
    Updates a packet capture schedule for devices in a Meraki organization using the Meraki Dashboard API.

    .DESCRIPTION
    Sends a PUT request to the Meraki Dashboard endpoint to update an existing packet capture schedule for devices in the specified organization.
    ...
    The ID of the packet capture schedule to be updated.
    ...
    Update-MerakiOrganizationDevicesPacketCaptureSchedule -AuthToken 'YOUR_API_KEY' -ScheduleID 'YOUR_SCHEDULE_ID' -ScheduleConfig $payload

    This example updates a daily packet capture schedule for the specified device and ports using a PowerShell hashtable converted to JSON.
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Put -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $ScheduleConfig
            return $response
        }
        catch {
            Write-Debug $_
