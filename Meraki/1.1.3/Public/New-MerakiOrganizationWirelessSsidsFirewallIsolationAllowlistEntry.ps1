function New-MerakiOrganizationWirelessSsidsFirewallIsolationAllowlistEntry {
    <#
    .SYNOPSIS
    Creates a new wireless SSID firewall isolation allowlist entry for a Meraki organization.

    .DESCRIPTION
    New-MerakiOrganizationWirelessSSIDsFirewallIsolationAllowlistEntry adds a single allowlist entry to the wireless SSIDs firewall isolation allowlist for the specified Meraki organization by calling the Meraki Dashboard API.
    ...
    New-MerakiOrganizationWirelessSSIDsFirewallIsolationAllowlistEntry -AuthToken $token -OrganizationID $orgId -AllowlistEntryConfig $payload

    .NOTES
    - The function issues an HTTP POST to the Meraki API endpoint:
        https://api.meraki.com/api/v1/organizations/{organizationId}/wireless/ssids/firewall/isolation/allowlist/entries
