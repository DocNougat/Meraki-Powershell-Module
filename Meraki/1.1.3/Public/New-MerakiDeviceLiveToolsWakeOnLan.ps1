function New-MerakiDeviceLiveToolsWakeOnLan {
    ...
    try {
        ...
        $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/liveTools/wakeOnLan"

        $response = Invoke-RestMethod -Method Post -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $WakeOnLanDetails
        return $response
    }
    catch {
        Write-Debug $_
