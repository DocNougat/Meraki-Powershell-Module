function New-MerakiDeviceLiveToolsArpTable {
    ...
    try {
        ...
        $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/liveTools/arpTable"

        $response = Invoke-RestMethod -Method Post -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $ArpTableDetails
        return $response
    }
    catch {
        Write-Debug $_
