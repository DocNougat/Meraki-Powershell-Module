function Invoke-MerakiDeviceLiveToolsPingDevice {
    ...
    try {
        ...
        $uri = "https://api.meraki.com/api/v1/devices/$DeviceSerial/liveTools/pingDevice"
        if ($Count) {
            $response = Invoke-RestMethod -Method Post -Uri $uri -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $body
        } else {
            $response = Invoke-RestMethod -Method Post -Uri $uri -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat"
        }
        return $response
    }
    catch {
