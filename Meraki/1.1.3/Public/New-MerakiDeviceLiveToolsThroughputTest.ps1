function New-MerakiDeviceLiveToolsThroughputTest {
    ...
    try {
        ...
        $url = "https://api.meraki.com/api/v1/devices/$DeviceSerial/liveTools/throughputTest"

        $response = Invoke-RestMethod -Method Post -Uri $url -headers $header -UserAgent "MerakiPowerShellModule/1.1.3 DocNougat" -Body $ThroughputTestDetails
        return $response
    }
    catch {
        Write-Debug $_
