function Get-MerakiNetworkPrefixesDelegatedStatic {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/prefixes/delegated/statics/$staticDelegatedPrefixId" -Header $header
        return $response
    }
    catch {
        Write-Error $_
