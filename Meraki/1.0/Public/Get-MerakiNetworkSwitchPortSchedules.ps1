function Get-MerakiNetworkSwitchPortSchedules {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$networkId/switch/portSchedules" -Header $header
        return $response
    } catch {
        Write-Error "Failed to retrieve port schedules for network '$networkId'. Error: $_"
    }
}
