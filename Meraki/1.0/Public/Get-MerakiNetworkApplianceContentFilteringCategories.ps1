function Get-MerakiNetworkApplianceContentFilteringCategories {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/contentFiltering/categories" -Header $header
        return $response
    } catch {
        Write-Error "Failed to retrieve content filtering categories for network '$NetworkId': $_"
    }
}