function Get-MerakiNetworkApplianceWarmSpare {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/warmSpare" -Header $header
        return $response
    }
    catch {
        Write-Error "Failed to retrieve warm spare configuration for network $NetworkId. $_"
    }
}
