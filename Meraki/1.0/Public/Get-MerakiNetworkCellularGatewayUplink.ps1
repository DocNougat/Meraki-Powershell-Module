function Get-MerakiNetworkCellularGatewayUplink {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/cellularGateway/uplink" -Header $header
        return $response
    }
    catch {
        Write-Error "Error occurred while retrieving uplink information: $_"
    }
}
