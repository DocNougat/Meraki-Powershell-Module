function Get-MerakiNetworkApplianceTrafficShapingCPCs {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/trafficShaping/customPerformanceClasses" -Header $header
        return $response
    }
    catch {
        Write-Error $_
