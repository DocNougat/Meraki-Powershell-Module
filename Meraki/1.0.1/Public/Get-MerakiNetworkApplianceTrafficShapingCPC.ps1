function Get-MerakiNetworkApplianceTrafficShapingCPC {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/trafficShaping/customPerformanceClasses/$CustomPerformanceClassId" -Header $header -ErrorAction Stop
        return $response
    }
    catch {
        Write-Error "An error occurred while retrieving the custom performance class settings for custom performance class '$CustomPerformanceClassId' in network '$NetworkId': $_"
    }
}
