...
#>
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$networkId/switch/mtu" -Header $header
        return $response
    } catch {
        Write-Error "Failed to retrieve MTU for network '$networkId'. Error: $_"
    }
}
