function Get-MerakiNetworkApplianceSsids {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/ssids" -Header $header -UserAgent "MerakiPowerShellModule/1.0.2 DocNougat" -ErrorAction Stop
        return $response
    }
    catch {
        Write-Error "An error occurred while retrieving the SSIDs for network '$NetworkId': $_"
    }
}
