function Get-MerakiNetworkWebhooksPayloadTemplate {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/webhooks/payloadTemplates/$payloadTemplateId" -Header $header -UserAgent "MerakiPowerShellModule/1.0.2 DocNougat"
        return $response
    }
    catch {
        Write-Error "Failed to retrieve payload template with ID '$payloadTemplateId' for network '$NetworkId'. $_"
    }
}    