function Get-MerakiNetworkSwitchRoutingMulticast {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$networkId/switch/routing/multicast" -Header $header -UserAgent "MerakiPowerShellModule/1.0.2 DocNougat"
        return $response
    } catch {
        Write-Error "Failed to retrieve multicast routing configuration for network '$networkId'. Error: $_"
    }
}
