function Get-MerakiOrganizationCameraCustomAnalyticsArtifact {
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/organizations/$OrganizationID/camera/customAnalytics/artifacts/$artifactId" -headers $header -UserAgent "MerakiPowerShellModule/1.0.8 DocNougat"
            return $response
        }
        catch {
            Write-Debug $_
