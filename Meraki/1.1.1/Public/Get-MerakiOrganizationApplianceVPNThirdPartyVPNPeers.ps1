function Get-MerakiOrganizationApplianceVPNThirdPartyVPNPeers {
    ...
    If($OrganizationID -eq "Multiple organizations found. Please specify an organization ID.") {
        ...
        try {
            ...
            $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/organizations/$OrganizationID/appliance/vpn/thirdPartyVPNPeers" -headers $header -UserAgent "MerakiPowerShellModule/1.1.1 DocNougat"
            return $response
        }
        catch {
        Write-Debug $_
