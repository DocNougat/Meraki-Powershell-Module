function Get-MerakiNetworkApplianceFirewallOneToOneNatRules {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/firewall/oneToOneNatRules" -headers $header -UserAgent "MerakiPowerShellModule/1.0.8 DocNougat" -ErrorAction Stop
        return $response
    }
    catch {
        Write-Debug $_
