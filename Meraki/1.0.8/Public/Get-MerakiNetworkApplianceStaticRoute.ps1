function Get-MerakiNetworkApplianceStaticRoute {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$NetworkId/appliance/staticRoutes/$StaticRouteId" -headers $header -UserAgent "MerakiPowerShellModule/1.0.8 DocNougat" -ErrorAction Stop
        return $response
    }
    catch {
        Write-Debug $_
