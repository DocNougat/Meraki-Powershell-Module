function Get-MerakiNetworkWirelessSsidSchedules {
    <#
    .SYNOPSIS
    Retrieves schedules for a wireless SSID in a Meraki network.

    .DESCRIPTION
    The Get-MerakiNetworkWirelessSsidSchedules function retrieves the schedules for a wireless SSID in a Meraki network using the Meraki Dashboard API. You must provide an API key and the ID of the network and SSID.
    ...
    try {
        ...
        $response = Invoke-RestMethod -Method Get -Uri "https://api.meraki.com/api/v1/networks/$networkId/wireless/ssids/$SSIDNumber/schedules" -headers $header -UserAgent "MerakiPowerShellModule/1.1.2 DocNougat"
        return $response
    } catch {
        Write-Debug $_
        Throw $_
